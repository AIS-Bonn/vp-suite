...
def visualize_video(dataset, video_in_length, video_pred_length, pred_model=None, out_dir=".",
    ...
    for i in range(num_vis):
        ...
        if vid_type[0] == "masks":
            gt_traj_vis = postprocess_mask(gt_traj)
            gt_traj_vis = colorize_semseg(gt_traj_vis, num_classes=vid_type[1])
        else:
        ...
        if pred_model is not None:
            ...
            with torch.no_grad():
                ...
                if vid_type[0] == "masks":
                    ...
                    pr_traj_vis = colorize_semseg(pr_traj_vis, num_classes=vid_type[1])
                else:
