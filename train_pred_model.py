...
def main(args):
    ...
    train_dir = os.path.join(data_dir, 'train', cfg.in_type)
    val_dir = os.path.join(data_dir, 'val', cfg.in_type)
    test_dir = os.path.join(data_dir, 'test', cfg.in_type)
    ...
    if cfg.model == "unet":
        ...
        pred_model = UNet3d(in_channels=vid_type[1], out_channels=vid_type[1], time_dim=VIDEO_IN_LENGTH).to(DEVICE)
    elif cfg.model == "lstm":
        print("prediction model: LSTM")
        pred_model = LSTMModel(in_channels=vid_type[1], out_channels=vid_type[1]).to(DEVICE)
    else:
    ...
    losses = {
        "fvd": (FrechetVideoDistance(num_frames=VIDEO_TOT_LENGTH, in_channels=vid_type[1]), True, 0.0),
        "mse": (MSE(), False, 1.0),
    ...
    for i in range(0, NUM_EPOCHS):
        ...
        print("Validating...")
        val_losses = validate_video_model(valid_loader, pred_model, DEVICE, VIDEO_IN_LENGTH, VIDEO_PRED_LENGTH, losses)
        ...
        visualize_video(val_data, VIDEO_IN_LENGTH, VIDEO_PRED_LENGTH, pred_model, out_dir, vid_type, num_vis=10)
    ...
    print("Saving visualizations...")
    visualize_video(test_data, VIDEO_IN_LENGTH, VIDEO_PRED_LENGTH, best_model, out_dir, vid_type, num_vis=10)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model", type=str, choices=["unet", "lstm", "copy"], help="Which model arch to train")
    parser.add_argument("--in-type", type=str, choices=["rgb", "masks"], default="rgb",
